...
namespace Slang.CodeAnalysis.Text
{
    partial class SourceText
    {
        ...
        public static SourceText From(string text, Encoding? encoding = null) =>
            text == null ? throw new ArgumentNullException(nameof(text)) : new StringText(text, encoding);
        ...
        public static SourceText From(Stream stream, Encoding? encoding = null, bool throwIfBinaryDetected = false)
        {
            ...
            return throwIfBinaryDetected && IsBinary(text) 
                ? throw new InvalidDataException() 
                : new StringText(text, encoding);
        }
        ...
        private static string Decode(Stream stream, Encoding encoding, out Encoding actualEncoding)
        {
            ...
            using var reader = new StreamReader(
                stream, encoding, detectEncodingFromByteOrderMarks: true, bufferSize: Math.Min(4096, length), leaveOpen: true);
